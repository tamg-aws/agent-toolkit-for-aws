...
description: >
  ...
  a failed SRA Verify build. Do NOT use for Prowler resource-configuration
  assessment (deploying-security-assessments), choosing which security services
  a workload warrants, or summarizing findings already in a security service.
version: 1
---
...
turn on, and it does not summarize findings that already exist in a security service
(`aws-security`). SRA Verify and SATv2 are the two tools in the SHIP workshop and are
frequently deployed together; they share the delegated-administrator prerequisite and
nothing else.
