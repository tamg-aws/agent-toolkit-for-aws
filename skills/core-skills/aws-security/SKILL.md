---
name: aws-security
description: "AWS security service suitability, coverage and cost recommendations: WAF/origin overlap, Shield Advanced justification, hybrid DNS and network protection, private authenticated application testing readiness, certificate renewal ownership and monitoring, incident response triage ownership. Also factual configuration and findings for GuardDuty, Inspector, Security Hub V2/OCSF (Exposure, connectors, automation), Security Hub CSPM/ASFF (standards, controls), Macie, Detective, Security Lake, AWS Security Incident Response membership and case coverage, and Organizations security policies. Use for advisory security-service choices even when one service is named. Use specialist skills for implementation, rule/code authoring, deployment, scans or troubleshooting outside these factual procedures. Supports local AWS CLI and AWS MCP."
metadata:
  version: "2"
---
