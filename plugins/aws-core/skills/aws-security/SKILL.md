...
**Reference loading:** When installed locally, resolve registry paths from this aws-security directory and Markdown links relative to their containing file. When loaded through MCP, retrieve skill `aws-security` through `retrieve_skill` with `file="references/service-recommendations.md"`, or the corresponding registry path. Nested support files use `file="references/service-recommendations/inventory-commands.md"` (and the other support filenames). Service applicability and focused evidence are in `file="references/service-recommendations/recommendation-matrix.md"`; follow its guide links for deeper guidance. Local links and MCP file paths address the same aws-security tree. All MCP `file` paths are relative to the aws-security root; do not read MCP-only files from the local filesystem.
